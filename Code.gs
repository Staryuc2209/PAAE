...
function guardarArchivos(carpetaAspirante, formData, curp, folio) {
  const resultado = {
    identificacion:    '', nombreIdentificacion: '',
    titulo:            '', nombreTitulo:         '',
    cedula:            '', nombreCedula:         '',
    certificado:       '', nombreCertificado:    '',
    boleta:            '', nombreBoleta:         '',
    cursoNinias:       '', nombreCursoNinias:    '',
    certifNinias:      '', nombreCertifNinias:   '',
    carta:             '', nombreCarta:          '',
    urlsPeriodos:      [],
    nombresPeriodos:   [],
    urlsActualizacion: [],
  ...
  if (formData.archivosRaw.identificacion) {
    const tipoId = (formData.datosIdentificacion.tipoIdentificacion || 'IDENTIFICACION').replace(/ /g,'_');
    const nom = pre + '_IDENTIFICACION_' + tipoId + '.pdf';
    resultado.nombreIdentificacion = nom;
  ...
  if (formData.archivosRaw.certificado) {
    ...
    resultado.nombreCertificado = nom;
  ...
  if (formData.archivosRaw.cursoNinias) {
    ...
    resultado.nombreCursoNinias = nom;
  ...
  if (formData.archivosRaw.certifNinias) {
    ...
    resultado.nombreCertifNinias = nom;
...
function registrarAspirante(hoja, formData, antiguedad, puntajes, urls, folio) {
  const encabezados = [
    ...
    'CURP','RFC','CORREO','TELEFONO','TIPO_IDENTIFICACION',
    'FUNCION','NIVEL_ACADEMICO','PROMEDIO',
    'ANIOS_SEGEY','MESES_SEGEY','PTJE_ANTIGUEDAD',
    'ANIOS_SEGEY','MESES_SEGEY','PTJE_SEGEY',
  ...
  hoja.appendRow([
    ...
    formData.datosIdentificacion.tipoIdentificacion || '',
    formData.datosLaborales.funcion,
    formData.formacion.nivel,
    formData.formacion.promedio || '',   // vacío si no se ingresó
    antiguedad.segey.anios, antiguedad.segey.meses,  parseFloat(antiguedad.segey.puntaje.toFixed(4)),
    antiguedad.rango.anios, antiguedad.rango.meses,  parseFloat(antiguedad.rango.puntaje.toFixed(4)),
    antiguedad.funcionEspec.anios, antiguedad.funcionEspec.meses, parseFloat(antiguedad.funcionEspec.puntaje.toFixed(4)),
    puntajes.reconocimiento, puntajes.formacion, puntajes.actualizacion, parseFloat(puntajes.total.toFixed(4)),
...
function enviarCorreoConfirmacion(formData, antiguedad, curp, folio, urls) {
  try {
    ...
    function filaDoc(nombre, url) {
      if (!url) return '';
      return '<tr><td>📄 <a href="' + url + '" style="color:#222222;text-decoration:none;font-weight:400;">' + nombre + '</a></td></tr>';
    }

    let filasDocIdentif = filaDoc(urls.nombreIdentificacion || 'Identificación Oficial', urls.identificacion);
    let filasDocFormacion = [
      filaDoc(urls.nombreTitulo       || 'Título Profesional',         urls.titulo),
      filaDoc(urls.nombreCedula       || 'Cédula Profesional',         urls.cedula),
      filaDoc(urls.nombreCertificado  || 'Certificado de Estudios',    urls.certificado),
      filaDoc(urls.nombreBoleta       || 'Boleta de Calificaciones',   urls.boleta),
      filaDoc(urls.nombreCursoNinias  || 'Curso Práctico Afín',        urls.cursoNinias),
      filaDoc(urls.nombreCertifNinias || 'Certificación Oficial Afín', urls.certifNinias)
    ].join('');
    ...
    let filasDocCarta = filaDoc(urls.nombreCarta || 'Carta de Reconocimiento al Desempeño', urls.carta);
...
<div class="c">
  ...
  <div class="b">
    ...
    <table>
      ...
      <tr><td><strong>Tipo de identificación</strong></td><td>${formData.datosIdentificacion.tipoIdentificacion || '—'}</td></tr>
    ...
    <div class="av"><strong>⚠️ Importante:</strong> Conserve este correo y su folio <strong>${folio}</strong> como comprobante de registro. Los documentos listados arriba están disponibles en los hipervínculos correspondientes. El expediente está sujeto a revisión y validación.</div>
